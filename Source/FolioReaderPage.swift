...
open class FolioReaderPage: UICollectionViewCell, UIWebViewDelegate, UIGestureRecognizerDelegate {
    ...
    open var wkWebView: FolioReaderWebView?
    ...
    public func setup(withReaderContainer readerContainer: FolioReaderContainer) {
        ...
        if wkWebView == nil {
            wkWebView = FolioReaderWebView(frame: webViewFrame(), readerContainer: readerContainer)
            wkWebView?.autoresizingMask = [.flexibleWidth, .flexibleHeight]
            wkWebView?.dataDetectorTypes = .link
        ...
        if colorView == nil {
            ...
            wkWebView?.scrollView.addSubview(colorView)
        }

        // Remove all gestures before adding new one
        wkWebView?.gestureRecognizers?.forEach({ gesture in
            wkWebView?.removeGestureRecognizer(gesture)
        })
        let tapGestureRecognizer = UITapGestureRecognizer(target: self, action: #selector(handleTapGesture(_:)))
        tapGestureRecognizer.numberOfTapsRequired = 1
        tapGestureRecognizer.delegate = self
        wkWebView?.addGestureRecognizer(tapGestureRecognizer)
    }
    ...
    override open func layoutSubviews() {
        ...
        wkWebView?.setupScrollDirection()
    ...
    func loadHTMLString(_ htmlContent: String!, baseURL: URL!) {
        ...
        wkWebView?.alpha = 0
        wkWebView?.loadHTMLString(tempHtmlContent, baseURL: baseURL)
    }
    ...
    open func webViewDidFinishLoad(_ webView: UIWebView) {
        ...
        UIView.animate(withDuration: 0.2, animations: {webView.alpha = 1}, completion: { finished in
            ...
            self.wkWebView?.createMenu(options: false)
        })
        ...
        if gestureRecognizer.view is FolioReaderWebView {
            if otherGestureRecognizer is UILongPressGestureRecognizer {
                if UIMenuController.shared.isMenuVisible {
                    wkWebView?.setMenuVisible(false)
                }
                return false
            }
            return true
    ...
    @objc open func handleTapGesture(_ recognizer: UITapGestureRecognizer) {
        ...
        if let _navigationController = self.folioReader.readerCenter?.navigationController, (_navigationController.isNavigationBarHidden == true) {
            let selected = wkWebView?.js("getSelectedText()")
    ...
    open func scrollPageToOffset(_ offset: CGFloat, animated: Bool) {
        ...
        wkWebView?.scrollView.setContentOffset(pageOffsetPoint, animated: animated)
    }
    ...
    open func scrollPageToBottom() {
        ...
        if bottomOffset.forDirection(withConfiguration: self.readerConfig) >= 0 {
            DispatchQueue.main.async {
                self.wkWebView?.scrollView.setContentOffset(bottomOffset, animated: false)
            }
        }
    }
    ...
    func getAnchorOffset(_ anchor: String) -> CGFloat {
        ...
        if let strOffset = wkWebView?.js("getAnchorOffset('\(anchor)', \(horizontal.description))") {
    ...
    func audioMarkID(_ identifier: String) {
        ...
        currentPage.wkWebView?.js("audioMarkID('\(playbackActiveClass)','\(identifier)')")
    }

    // MARK: UIMenu visibility

    override open func canPerformAction(_ action: Selector, withSender sender: Any?) -> Bool {
        guard let webView = wkWebView else { return false }
    ...
    fileprivate func setupClassBasedOnClickListeners() {
        for listener in self.readerConfig.classBasedOnClickListeners {
            self.wkWebView?.js("addClassBasedOnClickListener(\"\(listener.schemeName)\", \"\(listener.querySelector)\", \"\(listener.attributeName)\", \"\(listener.selectAll)\")");
        }
    }
    ...
    open func performJavaScript(_ javaScriptCode: String) -> String? {
        return wkWebView?.js(javaScriptCode)
    }
}
